...
def gemini_translate_and_analyze(news: list, indices: dict, btc: dict | None) -> dict | None:
    ...
    prompt = f"""당신은 한국에 거주하는 직장인·투자 초보자를 위한 경제 해설 전문가입니다.
미국 시장 뉴스가 '한국에 사는 나'에게 어떤 영향을 주는지 구체적으로 설명합니다.
...
### 1. 헤드라인 번역
각 헤드라인을 자연스럽고 이해하기 쉬운 한국어로 번역.

### 2. 경제적 해석 (2~3문장)
- 오늘 뉴스가 글로벌 경제에서 어떤 의미인지 설명
- 미국의 결정이 왜 한국 경제에도 영향을 주는지 연결고리 설명
- 전문용어는 반드시 괄호 안에 쉬운 설명 추가

### 3. 금리 시사점 (2~3문장)
- 미국 연준(Fed) 금리 방향이 한국은행 기준금리에 미치는 영향 설명
